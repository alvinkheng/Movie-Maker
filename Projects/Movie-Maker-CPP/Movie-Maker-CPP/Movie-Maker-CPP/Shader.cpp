...
void Shader::storeAttribLocations(const char** names, int numAttributes) {
    for (int i = 0; i < numAttributes; i++) {
        GLuint location = glGetAttribLocation(programID_, names[i]);
        attribLocations_[names[i]] = location;
        glEnableVertexAttribArray(location);
    }
}

void Shader::storeUniformLocations(const char** names, int numUniforms) {
    for (int i = 0; i < numUniforms; i++) {
        uniformLocations_[names[i]] = glGetUniformLocation(programID_, names[i]);
    }
}

GLuint Shader::getAttribLocationOf(const char* name) {
    return attribLocations_.find(name)->second;
}

GLuint Shader::getUniformLocationOf(const char* name) {
    return uniformLocations_.find(name)->second;
}