...
void MultiKinectScanner::run()
{
    ...
    while (!threadShouldExit())
    {
        ...
        {
            if (m_replaying) {
                if (!grabber->incrementCurrentImageIndex()) {
                    m_replaying = false;
...
void MultiKinectScanner::togglePlay(bool active) {
    m_replaying = active;
}

bool MultiKinectScanner::setCurrentReplayFrame(int frame) {
    bool indexInBounds = true;
    foreach_it(it, m_grabbers, std::set<RGBDGrabber*>)
    {
        indexInBounds &= (*it)->setCurrentImageIndex(frame);
    }
...
int MultiKinectScanner::getNumRecordedFrames() {
    foreach_const_it(it, m_grabbers, std::set<ntk::RGBDGrabber*>)
    {
        return (*it)->getImageListSize();
    }
    return 0;
}
